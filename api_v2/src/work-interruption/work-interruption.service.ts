...
export class WorkInterruptionService {
  ...
  async create(interruption: CreateInterruptionRequestInput) {
    ...
    try {
      ...
      const has_interruptionType = await this.prisma.workInterruptionType.findUnique({
        where: { id: interruption.workInterruptionTypeId }
      });
  
      if (!has_timeEntry || !has_interruptionType) {
        throw new NotFoundException('TimeEntry or WorkInterruptionType not found');
      }
      ...
      const createdInterruption = await this.prisma.workInterruption.create({
        data: {
          ...
          timeOut: interruption.timeOut ? moment(interruption.timeOut, 'HH:mm:ss').toString() : null,
          timeIn: interruption.timeIn ? moment(interruption.timeIn, 'HH:mm:ss').toString() : null,
      ...
      throw new InternalServerErrorException('An error occurred while creating the interruption');
    }
  }
